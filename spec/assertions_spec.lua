describe("Test Assertions", function()
  ...
  it("Checks asserts return all their arguments on success", function()
    local f = function() error("err message") end
    ...
    assert.is_same({".*", "string", "message"}, {assert.is_match(".*", "string", "message")})
    assert.is_same({f, "err message"}, {assert.has_error(f, "err message")})
  end)
